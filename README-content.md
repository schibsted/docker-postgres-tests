...
This image extends all [official postgres images](https://hub.docker.com/r/library/postgres/) with *testing dedicated* settings.

It adds:
- A generic healthcheck for tests to wait on (like using the `docker_await` strategy from [arquillian](http://arquillian.org/arquillian-cube/#_await))
<!-- This is a target, not done yet
- Some test specific tuning (mostly remove settings related to disk flushing)
-->

This image is runnable with the same interface as the official images, but with healthcheck management.

<!--
The section below is copy-pasted from https://raw.githubusercontent.com/docker-library/docs/master/postgres/README.md and adapted to match image names
-->
...
```console
$ docker run --name some-postgres -e POSTGRES_PASSWORD=mysecretpassword -d schibsted/postgres-test
```
...
```console
$ docker run --name some-postgres -e POSTGRES_PASSWORD_FILE=/run/secrets/postgres-passwd -d schibsted/postgres-test
```
...
```console
$ docker run -it --rm --user www-data schibsted/postgres-test
The files belonging to this database system will be owned by user "www-data".
...

$ docker run -it --rm --user 1000:1000 schibsted/postgres-test
...
1.	bind-mount `/etc/passwd` read-only from the host (if the UID you desire is a valid user on your host):
	...
	```console
	$ docker run -it --rm --user "$(id -u):$(id -g)" -v /etc/passwd:/etc/passwd:ro schibsted/postgres-test
...
2.	initialize the target directory separately from the final runtime (with a `chown` in between):
	...
	$ docker run -it --rm -v pgdata:/var/lib/postgresql/data schibsted/postgres-test
	The files belonging to this database system will be owned by user "postgres".
	...
	( once it's finished initializing successfully and is waiting for connections, stop it )
	$ docker run -it --rm -v pgdata:/var/lib/postgresql/data bash chown -R 1000:1000 /var/lib/postgresql/data
	$ docker run -it --rm --user 1000:1000 -v pgdata:/var/lib/postgresql/data schibsted/postgres-test
...
```dockerfile
FROM schibsted/postgres-test:9.4
...
-	Use a custom config file. Create a config file and get it into the container. If you need a starting place for your config file you can use the sample provided by PostgreSQL which is available in the container at `/usr/share/postgresql/postgresql.conf.sample`.
	...
	$ docker run -d --name some-postgres -v "$PWD/my-postgres.conf":/etc/postgresql/postgresql.conf schibsted/postgres-test -c 'config_file=/etc/postgresql/postgresql.conf'
	```
...
-	Set options directly on the run line. The entrypoint script is made so that any options passed to the docker command will be passed along to the `postgres` server daemon. From the [docs](https://www.postgresql.org/docs/current/static/app-postgres.html) we see that any option available in a `.conf` file can be set via `-c`.
	...
	```console
	$ docker run -d --name some-postgres schibsted/postgres-test -c 'shared_buffers=256MB' -c 'max_connections=200'
	```
...
## `schibsted/postgres-test:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `schibsted/postgres-test:alpine`
